...
/**
 * Servlet implementation class Import_exportGuest
 */

public class Import_exportGuest extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private IServiceAdmin admin; 
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Import_exportGuest() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getParameter("import_export") != null) {
			...
			switch (item) {
			case "import":
				isPerformed = admin.importGuestsList(nameFile);
				break;
			case "export":
				isPerformed = admin.exportGuestsList(nameFile);
				break;
			}
	}
