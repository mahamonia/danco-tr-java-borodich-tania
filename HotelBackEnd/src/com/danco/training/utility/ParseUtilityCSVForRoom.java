...
public class ParseUtilityCSVForRoom {
	
	private static ParseUtilityCSVForRoom utility;
	private CSVFileWorker fileRoom = new CSVFileWorker("room.csv");
	
	private ParseUtilityCSVForRoom(){
		
	}
	
	public static ParseUtilityCSVForRoom getInstance() {
		if (utility == null) {
			utility = new ParseUtilityCSVForRoom();
		}
		return utility;
	}
	...
	public List<Room> importData() { // read from CSV
		...
		List<Guest> listGuestForRoom = new ArrayList<>();
		...
		for (int i = 0; i < tempRoom.length; i++) {
			...
			for (int j = 0; j < str.length; j++) { 
				Guest guest = ServiceAdmin.getInstance().getGuestById(Integer.parseInt(str[j]));
